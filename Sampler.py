...
for result in tqdm(p.imap_unordered(samplerRun, cols), total=len(cols)):
    ...
    grp.attrs["class"] = np.void(pickle.dumps(result[1], pickle.HIGHEST_PROTOCOL))

    # Dump chain
    dset = grp.create_dataset("chain", data=result[0].chain, compression="gzip", compression_opts=9, dtype='f2')

    # Dump log-probabilities
    grp.create_dataset("lnprob", data=result[0].lnprobability, compression="gzip", compression_opts=9, dtype='f2')
