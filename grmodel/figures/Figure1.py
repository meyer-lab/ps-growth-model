...
def RangePlot(_df, var, low, high, ax):
    ...
    ax.fill_between(x_unique, y_high, y_low, facecolor='b', alpha=0.5)
...
def DataFitCheckFigureMaker(DoseResponseM, ax1, ax2, ax3, ax4, ax5):
    ...
    for n in range(len(df.index)):
        ...
        growthV = df['Emin_growth'].iloc[n] + ((df['Emax_growth'].iloc[n]
                                                - df['Emin_growth'].iloc[n]) * drugTerm)
    ...
    CIPlot(X, lObs, 0, ax2)  # Compare with the real data
    ...
    ax3.set_ylabel('growthRate')
    ax4.set_xlabel('concentration')
    ax4.set_ylabel('deathRate')
...
def PCAFigureMaker(self, ax):
    ...
    ax.scatter(X_new[:, 0], X_new[:, 1], alpha=0.5)
    ...
    # for i in range(n):
    #    ax.arrow(0, 0, coeff[i, 0], coeff[i, 1], color='black')
